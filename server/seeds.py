...
from app.schemas.user_schema import UserSchema  # Import UserSchema
...
with app.app_context():
    ...
    if not UserProfile.query.first():
        profiles = [
            UserProfile(user_id=1, points=500, xp=1000, bio="Admin user", avatar_url="https://example.com/avatar1.png", last_active=datetime(2024, 6,3)),
            UserProfile(user_id=2, points=300, xp=600, bio="Admin user", avatar_url="https://example.com/avatar2.png", last_active=datetime(2024, 1, 5)),
            UserProfile(user_id=3, points=100, xp=200, bio="Learner user", avatar_url="https://example.com/avatar3.png", last_active=datetime(2024, 9, 1)),
            UserProfile(user_id=4, points=250, xp=400, bio="Contributor user", avatar_url="https://example.com/avatar4.png", last_active=datetime(2024, 6, 7)),
            UserProfile(user_id=5, points=150, xp=300, bio="Learner user", avatar_url="https://example.com/avatar5.png", last_active=datetime(2024, 11, 8)),
            UserProfile(user_id=6, points=50, xp=100, bio="Learner user", avatar_url="https://example.com/avatar6.png", last_active=datetime(2024, 2, 5)),
        ]
    ...
    #         Progress(user_id=3, module_id=1, completed=False),
    #         Progress(user_id=4, module_id=2, completed=True),
    #         Progress(user_id=5, module_id=2, completed=False),
    #         Progress(user_id=6, module_id=1, completed=False)
    #     ]
    ...
    if not UserLearningPath.query.first():
        user_learning_paths = [
            UserLearningPath(user_id=1, learning_path_id=1, progress=0.5, date_enrolled=datetime(2024, 11, 6)),
            UserLearningPath(user_id=2, learning_path_id=2, progress=0.2, date_enrolled=datetime(2024, 11, 6)),
            UserLearningPath(user_id=3, learning_path_id=3, progress=0.1, date_enrolled=datetime(2024, 11, 6)),
            UserLearningPath(user_id=4, learning_path_id=4, progress=0.7, date_enrolled=datetime(2024, 11, 6)),
            UserLearningPath(user_id=5, learning_path_id=5, progress=0.3, date_enrolled=datetime(2024, 11, 6)),
            UserLearningPath(user_id=6, learning_path_id=6, progress=0.05, date_enrolled=datetime(2024, 11, 6))
        ]
    ...
    if not Module.query.first():
        modules = [
            Module(title="Module 1", description="Introduction", learning_path_id=1),
            Module(title="Module 2", description="Advanced Topics", learning_path_id=1),
            Module(title="Module 1", description="Basics of Web Development", learning_path_id=2),
        ]
